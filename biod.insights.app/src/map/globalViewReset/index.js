...
function init({ map: _map }) {
  ...
  $reset = window.jQuery('#gd-global-view-reset');
  let icon = getGlobalViewVector();

  $reset.append(window.jQuery(icon));
...
function isGlobalView() {
  const [longitude, latitude] = esriMap.extent
    ? [esriMap.extent.getCenter().getLongitude(), esriMap.extent.getCenter().getLatitude()]
    : MAP_CENTER;

  return (
    GLOBAL_ZOOM_LEVEL === esriMap.getZoom() &&
