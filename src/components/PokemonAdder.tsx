...
const PokemonAdder = () => {
  ...
  const localPokemonData = JSON.parse(
    localStorage.getItem("savedPokemon") as string
  ) as TypePokemon[];
  ...
  async function getSavedPokemonFromLocalStorage() {
    ...
    if (localPokemonData) {
      const promises = localPokemonData.map((p) =>
        PokeApi.getPokemon(p.pokemon.url)
      );
  ...
  const handleChange = async (value: string) => {
    ...
    if (pokemonTypes && pokemonTypes.some((e) => e.name === value)) {
      ...
      await PokeApi.getPokemonByType(value).then((r) => {
        ...
        const promises = r.pokemon.map((p) =>
          PokeApi.getPokemon(p.pokemon.url)
        );
  ...
  const handleAdd = () => {
    ...
    setSearchQuery("");
    setSavedPokemon(filteredPokemon);
    setFilteredPokemon([]);
  };

  const handleRemove = (pokemon: ProcessedPokemon) => {
    const newSavedPokemonData = localPokemonData.filter(
      (p) => !(p.pokemon.name === pokemon.species.name)
    );
    const newPokemonData = savedPokemon.filter(
      (p) => !(p.species.name === pokemon.species.name)
    );
    ...
    <section>
      <div className="SearchContainer">
        <Input
          ...
          placeholder="Query for “type” required e.g. (fire, water)"
