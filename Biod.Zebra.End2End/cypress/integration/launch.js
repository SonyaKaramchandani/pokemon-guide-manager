describe("Launch", () => {
  ...
  it("landing page", () => {
    ...
    cy.get("#marketing").should("be.visible");
    ...
    cy.contains("Near real-time").should("be.visible");

    cy.get("#marketing-image-container").should("be.visible");

    cy.contains("Talk to a specialist").should("be.visible");
  });

  /* TODO: re enable this test after amending terms of service text see https://bluedotglobal.atlassian.net/browse/PT-388
...
*/
  ...
  it("forgot password", () => {
    ...
    cy.contains("Retrieve Your Password").should("be.visible");
    ...
    cy.contains("The Email field is not a valid e-mail address.").should(
      "be.visible"
    );
    ...
    cy.contains("A link has been sent to your email").should("be.visible");
  });

  it("Invalid login attempt", () => {
    cy.visit("/");

    cy.get("#Email").type("anyemail@example.com");
