...
BEGIN
	...
	If @GeonameIds=N''
		...
		FROM (Select * from surveillance.Event WHERE EventId=@EventId) as f1  
			...
			INNER JOIN (Select * from disease.Diseases WHERE DiseaseId=@DiseaseId) as f3 ON 1=1 
			LEFT JOIN (Select * from [zebra].[EventDestinationAirport] WHERE DestinationStationId=-1 and EventId=@EventId) as f4 ON 1=1
	...
	Begin --B
		...
		If @useUserAoi=1 --user user aoi
			...
			FROM (Select * from surveillance.Event WHERE EventId=@EventId) as f1  
				...
				INNER JOIN (Select * from disease.Diseases WHERE DiseaseId=@DiseaseId) as f3 ON 1=1
				LEFT JOIN (Select * from [zebra].[EventDestinationAirport] WHERE DestinationStationId=-1 and EventId=@EventId) as f4 ON 1=1
				LEFT JOIN (Select * from [zebra].[EventImportationRisksByUser] WHERE EventId=@EventId and UserId=@UserId) as f5 ON 1=1
		Else --use user input
		Begin--6
			--local event?
